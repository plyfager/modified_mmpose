...
class PoseLocalVisualizer(Visualizer):
    ...
    def __init__(self,
        ...
        self.dataset_meta = {}
        self.zero_set_image = zero_set_image
    ...
    def add_datasample(self,
        ...
        if draw_gt:
            # replace gt_image with all zero
            # gt_img_data = image.copy()
            gt_img_data = np.zeros_like(image)
            gt_img_heatmap = None
            ...
            if 'gt_instances' in data_sample:
                ...
                if draw_bbox:
                    ...
                    # record image name and information of new bounding box
                    H, W, _ = gt_img_data.shape
                    x1, y1, x2, y2 = data_sample.gt_instances.bboxes[0]
                    ratio = 2.2
                    res = 512
                    delta_w = 0.5 * (ratio-1) * (x2 - x1)
                    delta_h = 0.5 * (ratio-1) * (y2 - y1)   
                    x1 = int(max(0, x1 - delta_w))
                    x2 = int(min(x2 + delta_w, W))
                    y1 = int(max(0, y1 - delta_h))
                    y2 = int(min(y2 + delta_h, H))
                    with open("image_info.txt", "a") as f:
                        f.write(out_file + ' ' + f"{x1}, {y1}, {x2}, {y2}")
                    # crop image due to a certain scale and save it
                    cropped_map = gt_img_data[y1:y2, x1:x2, :]
                    cv2.imwrite(os.path.join('work_dirs/datasets/300w/cropped_data/map', filename), cv2.resize(cropped_map, (res, res)))
                    cropped_image = image[y1:y2, x1:x2, :]
                    cv2.imwrite(os.path.join('work_dirs/datasets/300w/cropped_data/image', filename), cv2.cvtColor(cv2.resize(cropped_image, (res, res)), cv2.COLOR_RGB2BGR))
                    return
                    # next: write a script to extract image and kpt map then resize them to 512x512
                    # and also generate prompt.json
                    # next: rerange the folder and start training
        ...
        if draw_pred:
            ...
            if self.zero_set_image:
                pred_img_data = np.zeros_like(image)
